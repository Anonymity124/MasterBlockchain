...
- [交易的输入输出](#交易的输入输出)
  - [交易输出](#交易输出)
    - [交易序列化 - 输出](#交易序列化---输出)
  - [交易输入](#交易输入)
    - [交易序列化--交易输入](#交易序列化--交易输入)
  - [交易费](#交易费)
  - [把交易费加到交易中](#把交易费加到交易中)
- [比特币交易脚本和脚本语言](#比特币交易脚本和脚本语言)
  - [图灵非完备性](#图灵非完备性)
  - [去中心化验证](#去中心化验证)
  - [脚本构建（锁定与解锁）](#脚本构建锁定与解锁)
    - [脚本执行堆栈](#脚本执行堆栈)
    - [一个简单的脚本](#一个简单的脚本)
    - [解锁和锁定脚本的单独执行](#解锁和锁定脚本的单独执行)
  - [P2PKH（Pay-to-Public-Key-Hash）](#p2pkhpay-to-public-key-hash)
- [数字签名（ECDSA）](#数字签名ecdsa)
  - [数字签名如何工作](#数字签名如何工作)
    - [创建数字签名](#创建数字签名)
    - [签名序列化（DER）](#签名序列化der)
  - [验证签名](#验证签名)
  - [签名哈希类型（SIGHASH）](#签名哈希类型sighash)
  - [ECDSA数学](#ecdsa数学)
  - [随机性在签名中的重要性](#随机性在签名中的重要性)
- [比特币地址，余额和其他摘要](#比特币地址余额和其他摘要)
- [多重签名](#多重签名)
- [P2SH（Pay-to-Script-Hash）](#p2shpay-to-script-hash)
  - [P2SH地址](#p2sh地址)
  - [P2SH的优点](#p2sh的优点)
  - [赎回脚本和标准确认](#赎回脚本和标准确认)
- [数据记录输出（RETURN操作符）](#数据记录输出return操作符)
- [时间锁（Timelocks）](#时间锁timelocks)
  - [交易锁定时间（nLocktime）](#交易锁定时间nlocktime)
    - [交易锁定时间限制](#交易锁定时间限制)
  - [检查锁定时间验证Check Lock Time Verify (CLTV)](#检查锁定时间验证check-lock-time-verify-cltv)
  - [相对时间锁](#相对时间锁)
  - [nSequence相对时间锁](#nsequence相对时间锁)
    - [nSequence的本义](#nsequence的本义)
    - [nSequence作为一个共同执行的相对时间锁定](#nsequence作为一个共同执行的相对时间锁定)
  - [带CSV的相对时间锁](#带csv的相对时间锁)
  - [中位时间过去Median-Time-Past](#中位时间过去median-time-past)
  - [针对费用狙击（Fee Sniping）的时间锁定](#针对费用狙击fee-sniping的时间锁定)
- [具有流量控制的脚本（条件子句 (Conditional Clauses)）](#具有流量控制的脚本条件子句-conditional-clauses)
  - [带有VERIFY操作码的条件子句](#带有verify操作码的条件子句)
  - [在脚本中使用流控制](#在脚本中使用流控制)
- [复杂的脚本示例](#复杂的脚本示例)
